// ESPSmart-Environment/ESPSmart-Environment.ino
...
void handleOverride() {
    ...
    AutomationAI::setManualOverride(v);
...
void loop() {
  ...
  if (now - lastLoop >= LOOP_INTERVAL) {
    ...
    // automationAI (toggle outputs)
    AutomationAI::evaluate(t, h, light, motion);

    // average readings
    SensorManager::averageReadings(t, h, light);
  }
}